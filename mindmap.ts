...
export function perspectivesToMarkdown(topic: string, perspectives: any[]): string {
  let markdown = `# ${topic}\n`;
...
export function addHoverFunctionality(container: HTMLElement) {
  container.addEventListener('mouseover', (event) => {
    const target = (event.target as HTMLElement).closest('.markmap-node');
    if (target) {
